...
export default function Page_2() {
  ...
  const deffunc = () => {
    if (activeId.id !== null && def) {
      ...
      axios.post('http://127.0.0.1:3002/getDefinition', state).then((res) => {
        if (res.data.length!==0) {
          setdef(false);
          res.data.map((p)=>{inputarr.push(p.definition)});
        }
        else {
          setdef(true);
        }
  ...
  const durfunc = () => {
    if (activeId.id !== null && dur) {
      const state = { id: activeId.id }
      axios.post('http://127.0.0.1:3002/getDuration', state).then((res) => {
        if (res.data.length!==0) {
          setdur(false);
          res.data.map((p)=>{inputarr1.push(p.duration)});
        }
        else {
          setdur(true);
        }
  ...
  const pushdef = () => {
    if (activeId.id === null)
      ...
      inputarr.map((p) => {
        ...
        axios.post('http://127.0.0.1:3002/addDefinition', uploadData).then((res) => {
  ...
  const pushdef1 = () => {
    if (activeId.id === null)
      ...
      inputarr1.map((p) => {
        const uploadData = { id: activeId.id, def: p }
        axios.post('http://127.0.0.1:3002/addDuration', uploadData).then((res) => {
